...
class MyBooksViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun refresh() {
        ...
        val url = "https://ubaya.fun/native/160419002/ulib/mybooks.php?user_id=${GlobalData.userID}"
        val stringRequest = StringRequest(
            ...
            url,
            {
                ...
                Log.d("my_books", result.toString())
            },
            {
                loadingLiveData.value = false
                booksLoadErrorLiveData.value = true
                Log.d("error_my_books", it.message.toString())
            }
        ).apply {
            tag = TAG
        }
